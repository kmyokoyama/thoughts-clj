...
(def ^:private http-client (atom nil))
...
(defn- stop-full-test-system
  [system]
  (a.repository.datomic/delete-database (-> system :config :datomic-uri))
...
(use-fixtures :each (fn [test]
                      (let [[start-system! stop-system!] (:in-mem start-stop-fns)
                            sys (start-system!)]
                        (swap! http-client (constantly (:http-client sys)))
                        (test)
                        (stop-system! sys))))
...
(defn signup
  ...
  (let [password (:password user)
        user-id (-> (http.helper/post! @http-client "signup" user) :result :id)]
    {:user-id user-id :password password}))

(defn login
  [{:keys [user-id password]}]
  (let [token (-> (http.helper/post! @http-client "login" {:user-id user-id :password password}) :result :token)]
...
(deftest ^:integration test-signup
  (testing "Sign up with a single user"
    (let [{:keys [response body]} (http.helper/post! @http-client "signup" (application.helper/random-user))]
  ...
  (testing "Sign up with duplicate email returns a failure"
    (let [first-user (application.helper/random-user)
      ...
      (http.helper/post! @http-client "signup" first-user)
      (let [second-user {:name (application.helper/random-fullname) :email first-user-email :username (application.helper/random-username) :password (application.helper/random-password)}
            {:keys [response body result]} (-> (http.helper/post! @http-client "signup" second-user))]
...
(deftest ^:integration test-login
  (testing "Login returns success when user exists"
    (let [user (application.helper/random-user)
          ...
          {:keys [response body result]} (http.helper/post! @http-client "login" {:user-id user-id :password password})]
      (is (= "success" (:status body)))
  ...
  (testing "Login fails when user is already logged in"
    (let [user-id (application.helper/random-uuid)
      ...
      (http.helper/post! @http-client "login" {:user-id user-id :password password}) ;; Log first time.
      (let [{:keys [response body result]} (http.helper/post! @http-client "login" {:user-id user-id :password password})]
        (is (= "failure" (:status body)))
        (is (= 400 (:status response)))                     ;; HTTP 400 Bad Request.
...
(deftest ^:integration test-logout
  (testing "Logout returns success when user is already logged in"
    (let [{:keys [token]} (signup-and-login)
          {:keys [response body]} (http.helper/post! @http-client "logout" token {})]
      (is (= "success" (:status body)))
      (is (= 200 (:status response)))))

  (testing "Logout fails when user is not logged in yet"
    (let [{:keys [response body]} (http.helper/post! @http-client "logout" nil {})]
      (is (= "failure" (:status body)))
...
(deftest ^:integration test-thought
  (testing "Add a single thought"
    (let [{:keys [user-id token]} (signup-and-login)
          ...
          {:keys [response body result]} (http.helper/post! @http-client "thought" token (application.helper/random-thought text))]
      (is (= "success" (:status body)))
      (is (= 201 (:status response)))                       ;; HTTP 201 Created.
...
(deftest ^:integration test-get-thoughts-from-user
  (testing "Get two thoughts from the same user"
    (let [{:keys [user-id token]} (signup-and-login)
          first-thought-id (-> (http.helper/post! @http-client "thought" token (application.helper/random-thought)) :result :id)
          second-thought-id (-> (http.helper/post! @http-client "thought" token (application.helper/random-thought)) :result :id)
  ...
  (testing "Returns no thought if user has not thought yet"
    (let [{:keys [user-id token]} (signup-and-login)]
      ;; No thought.
      (let [{:keys [response body result]} (http.helper/get! @http-client (str "user/" user-id "/thoughts") token)]
        (is (= "success" (:status body)))
        (is (= 200 (:status response)))                     ;; HTTP 200 OK.
...
(deftest ^:integration test-get-user-by-id
  (testing "Get an existing user returns successfully"
    (let [expected-user (application.helper/random-user)
          ...
          {:keys [response result]} (http.helper/get! @http-client (str "user/" user-id) token)
  ...
  (testing "Get a missing user returns failure"
    (let [{:keys [token]} (signup-and-login)
          ...
          {:keys [response body result]} (http.helper/get! @http-client (str "user/" user-id) token)]
      (is (= 400 (:status response)))                       ;; HTTP 400 Bad Request.
      (is (= "failure" (:status body)))
...
(deftest ^:integration test-get-thought-by-id
  (testing "Get an existing thought returns successfully"
    (let [{:keys [user-id token]} (signup-and-login)
          ...
          thought-id (-> (http.helper/post! @http-client "thought" token expected-thought) :result :id)
          {:keys [response result]} (http.helper/get! @http-client (str "thought/" thought-id) token)
  ...
  (testing "Get a missing thought returns failure"
    (let [{:keys [token]} (signup-and-login)
          ...
          {:keys [response body result]} (http.helper/get! @http-client (str "thought/" thought-id) token)]
      (is (= 400 (:status response)))                       ;; HTTP 400 Bad Request.
      (is (= "failure" (:status body)))
      (is (= "resource not found" (:type result)))
...
(deftest ^:integration test-like
  ...
  (testing "Like a missing thought returns failure"
    (let [{:keys [token]} (signup-and-login)
          thought-id (application.helper/random-uuid)
          {:keys [response body result]} (http.helper/post! @http-client (str "thought/" thought-id "/like") token {})]
      (is (= 400 (:status response)))                       ;; HTTP 400 Bad Request.
      (is (= "failure" (:status body)))
      (is (= "resource not found" (:type result)))
      (is (= "thought" (:subject result)))
...
(deftest ^:integration test-add-reply
  (testing "Add new reply to existing thought returns success"
    (let [{:keys [user-id token]} (signup-and-login)
          ...
          {:keys [response body result]} (http.helper/post! @http-client (str "thought/" thought-id "/reply") token {:text reply-text})]
      (is (= "success" (:status body)))
      (is (= 201 (:status response)))                       ;; HTTP 201 Created.
      (is (= user-id (:user-id result)))
...
(deftest ^:integration test-get-rethought-by-id
  (testing "Get rethoughts from thought not rethoughted yet returns an empty list"
    (let [{:keys [token]} (signup-and-login)
          thought-id (-> (http.helper/post! @http-client "thought" token (application.helper/random-thought)) :result :id)
          rethought-id (-> (http.helper/post! @http-client (str "thought/" thought-id "/rethought") token {}) :result :id)
          {:keys [response body result]} (http.helper/get! @http-client (str "rethought/" rethought-id) token)]
      (is (= 200 (:status response)))                       ;; HTTP 200 OK.
      (is (= "success" (:status body)))
...
(deftest ^:integration test-get-rethoughts
  (testing "Get rethoughts from a thought already rethoughted returns all replies"
    (let [{:keys [user-id token]} (signup-and-login)
          thought-id (-> (http.helper/post! @http-client "thought" token (application.helper/random-thought user-id)) :result :id)]
      (dotimes [_ 5] (http.helper/post! @http-client (str "thought/" thought-id "/rethought") token {}))
      (dotimes [_ 5] (http.helper/post! @http-client (str "thought/" thought-id "/rethought-comment") token {:comment (application.helper/random-text)}))
      (let [{:keys [response body result]} (http.helper/get! @http-client (str "thought/" thought-id "/rethoughts") token)]
        (is (= 200 (:status response)))                     ;; HTTP 200 OK.
        (is (= "success" (:status body)))
  ...
  (testing "Get rethoughts from thought not rethoughted yet returns an empty list"
    (let [{:keys [token]} (signup-and-login)
          thought-id (-> (http.helper/post! @http-client "thought" token (application.helper/random-thought)) :result :id)
          {:keys [response body result]} (http.helper/get! @http-client (str "thought/" thought-id "/rethoughts") token)]
      (is (= 200 (:status response)))                       ;; HTTP 200 OK.
      (is (= "success" (:status body)))
      (is (empty? result)))))
...
(deftest ^:integration test-get-replies
  (testing "Get rethoughts from a thought already rethoughted returns all replies"
    (let [{:keys [token]} (signup-and-login)
          thought-id (-> (http.helper/post! @http-client "thought" token (application.helper/random-thought)) :result :id)]
      (dotimes [_ 5] (http.helper/post! @http-client (str "thought/" thought-id "/reply") token {:text (application.helper/random-text)}))
      (let [{:keys [response body result]} (http.helper/get! @http-client (str "thought/" thought-id "/replies") token)]
        (is (= 200 (:status response)))                     ;; HTTP 200 OK.
        (is (= "success" (:status body)))
  ...
  (testing "Get replies from thought not replied yet returns an empty list"
    (let [{:keys [token]} (signup-and-login)
          thought-id (-> (http.helper/post! @http-client "thought" token (application.helper/random-thought)) :result :id)
          {:keys [response body result]} (http.helper/get! @http-client (str "thought/" thought-id "/replies") token)]
      (is (= 200 (:status response)))                       ;; HTTP 200 OK.
      (is (= "success" (:status body)))
      (is (empty? result)))))
...
(deftest ^:integration test-follow
  (testing "User follows another user"
    (let [{follower-id :user-id follower-token :token} (signup-and-login)
          ...
          {:keys [response body]} (http.helper/post! @http-client (str "user/" followed-id "/follow") follower-token {})
          followed-result (:result body)
          follower-result (-> (http.helper/get! @http-client (str "user/" follower-id) follower-token {}) :result)]
      (is (= 200 (:status response)))                       ;; HTTP 200 OK.
      (is (= "success" (:status body)))
  ...
  (testing "Follow a missing user returns failure"
    (let [{:keys [token]} (signup-and-login)
          ...
          {:keys [response body result]} (http.helper/post! @http-client (str "user/" random-followed-id "/follow") token {})]
      (is (= 400 (:status response)))                       ;; HTTP 400 Bad Request.
      (is (= "failure" (:status body)))
      (is (= "resource not found" (:type result)))
      (is (= "user" (:subject result)))
  ...
  (testing "Follow the same user twice returns failure"
    (let [{:keys [user-id token]} (signup-and-login)
          {followed-id :user-id} (signup-and-login)
          _ (http.helper/post! @http-client (str "user/" followed-id "/follow") token {})
          {:keys [response body result]} (http.helper/post! @http-client (str "user/" followed-id "/follow") token {})]
      (is (= 400 (:status response)))                       ;; HTTP 400 Bad Request.
      (is (= "failure" (:status body)))
      (is (= "invalid action" (:type result)))
...
(deftest ^:integration test-unfollow
  (testing "User unfollows an user she/he follows"
    (let [{follower-id :user-id follower-token :token} (signup-and-login)
          {followed-id :user-id} (signup-and-login)
          _ (http.helper/post! @http-client (str "user/" followed-id "/follow") follower-token {})
          {:keys [response body]} (http.helper/post! @http-client (str "user/" followed-id "/unfollow") follower-token {})
  ...
  (testing "Unfollow a missing user returns failure"
    (let [{:keys [token]} (signup-and-login)
          random-followed-id (application.helper/random-uuid)
          {:keys [response body result]} (http.helper/post! @http-client (str "user/" random-followed-id "/unfollow") token {})]
      (is (= 400 (:status response)))                       ;; HTTP 400 Bad Request.
      (is (= "failure" (:status body)))
      (is (= "resource not found" (:type result)))
      (is (= "user" (:subject result)))
  ...
  (testing "Unfollow an user that is not currently followed"
    (let [{:keys [user-id token]} (signup-and-login)
          {followed-id :user-id} (signup-and-login)
          {:keys [response body result]} (http.helper/post! @http-client (str "user/" followed-id "/unfollow") token {})]
      (is (= 400 (:status response)))                       ;; HTTP 400 Bad Request.
      (is (= "failure" (:status body)))
      (is (= "invalid action" (:type result)))
  ...
  (testing "Unfollow the same user twice returns failure"
    (let [{:keys [user-id token]} (signup-and-login)
          {followed-id :user-id} (signup-and-login)
          _ (http.helper/post! @http-client (str "user/" followed-id "/follow") token {})
          _ (http.helper/post! @http-client (str "user/" followed-id "/unfollow") token {}) ;; First unfollow.
          {:keys [response body result]} (http.helper/post! @http-client (str "user/" followed-id "/unfollow") token {})]
      (is (= 400 (:status response)))                       ;; HTTP 400 Bad Request.
      (is (= "failure" (:status body)))
      (is (= "invalid action" (:type result)))
...
(deftest ^:integration test-get-user-following
  (testing "Get following list of an user"
    (let [{follower-id :user-id follower-token :token} (signup-and-login)
          {followed-id :user-id} (signup-and-login)
          _ (http.helper/post! @http-client (str "user/" followed-id "/follow") follower-token {})
          {:keys [response body result]} (http.helper/get! @http-client (str "user/" follower-id "/following") follower-token {})]
      (is (= 200 (:status response)))                       ;; HTTP 200 OK.
      (is (= "success" (:status body)))
  ...
  (testing "Get following list of an user that does not follow anyone returns an empty list"
    (let [{:keys [user-id token]} (signup-and-login)
          {:keys [response body result]} (http.helper/get! @http-client (str "user/" user-id "/following") token {})]
      (is (= 200 (:status response)))                       ;; HTTP 200 OK.
      (is (= "success" (:status body)))
      (is (empty? result)))))
...
(deftest ^:integration test-get-user-followers
  (testing "Get followers list of an user"
    (let [{follower-token :token} (signup-and-login)
          {followed-id :user-id} (signup-and-login)
          _ (http.helper/post! @http-client (str "user/" followed-id "/follow") follower-token {})
          {:keys [response body result]} (http.helper/get! @http-client (str "user/" followed-id "/followers") follower-token {})]
      (is (= 200 (:status response)))                       ;; HTTP 200 OK.
      (is (= "success" (:status body)))
      (is (= 1 (:total body) (count result)))))
  ...
  (testing "Get followers list of an user that is not followed by anyone returns an empty list"
    (let [{:keys [user-id token]} (signup-and-login)
          {:keys [response body result]} (http.helper/get! @http-client (str "user/" user-id "/followers") token {})]
      (is (= 200 (:status response)))                       ;; HTTP 200 OK.
      (is (= "success" (:status body)))
      (is (empty? result)))))
...
(deftest ^:integration test-get-feed
  (testing "Get feed of an user returns most recent thoughts"
    (let [{first-user-id :user-id first-user-password :password} (signup (application.helper/random-user))
      ...
      (dotimes [_ 5] (http.helper/post! @http-client "thought" second-user-token (application.helper/random-thought)))

      (let [third-user-token (-> (login {:user-id third-user-id :password third-user-password}) :token)]
        (dotimes [_ 5] (http.helper/post! @http-client "thought" third-user-token (application.helper/random-thought)))

        (let [first-user-token (-> (login {:user-id first-user-id :password first-user-password}) :token)]
          (http.helper/post! @http-client (str "user/" second-user-id "/follow") first-user-token {})
          (http.helper/post! @http-client (str "user/" third-user-id "/follow") first-user-token {})

          (let [{:keys [response body result]} (http.helper/get! @http-client "feed" first-user-token {})]
            (is (= 200 (:status response)))                 ;; HTTP 200 OK.
            (is (= "success" (:status body)))
            (is (= 10 (:total body) (count result)))
